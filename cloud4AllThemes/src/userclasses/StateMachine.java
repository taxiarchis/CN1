...
public class StateMachine extends StateMachineBase {
	...
	private static String selectedTheme, selectedFont, selectedLanguage;
	...
	// "{\"info.cloud4all.JME\": {\"theme\": \"Yellow-Black\", \"fontSize\": \"veryLarge\", \"language\": \"English\", \"volume\": 50, \"vibrationMode\": 2}";
	// String dummyJsonResponse2 =
	// "{\"info.cloud4all.JME\": {\"fontSize\": \"large\", \"language\": \"Greek\", \"volume\": 0, \"vibrationMode\": 1}";
	// String dummyJsonResponse3 =
	// "{\"info.cloud4all.JME\": {\"language\": \"English\", \"volume\": 0, \"vibrationMode\": 1}";
	...
	public StateMachine(String resFile) {
		...
		// testingLog = FileSystemStorage.getInstance().getAppHomePath() + sep;

		// Log.getInstance().setFileWriteEnabled(true);
		// Log.getInstance().setFileURL("file:///Device/documents/cn1.log");
...
//		}
	...
	protected void initVars() { // TODO: TO BE COMMENTED IN
		...
		//
		// // is =
		// com.codename1.io.Storage.getInstance().createInputStream(photoPath);
		// fs.mkdir(roots[0] + "cloud4all"); // file:///E:/
		// stepDebug = 30;
		// char sep = fs.getFileSystemSeparator();
		// stepDebug = 31;
		// audioNameDebug = roots[0] + "cloud4all" + sep + getDateTime() +
		// ".wav";
	...
	private void setAppropriateRadioButtonsSelected(final Form f) {
		...
		if (selectedLanguage.equals("en")) {
			...
			findGermanRadioButton().setSelected(true);
		}
...
//								showForm("Main", null);
	...
	//
	// ContactSelector contactSelector = new ContactSelector(this); //new
	// ContactSelector(r, selectedTheme, selectedLanguage, selectedFont); // TAX
	// testForm.addComponent(contactSelector);
	// // testForm.show();
	// /*
	// * After creating the contactselector object and add it to your form, you
	// will then add a ContactListener to your object which will listen for the
	// done and cancel
	// * event(done event occurs after a user has finished selecting the
	// contact(s)). The doneContactSelection method in the ContactListener is
	// the corresponding done function
	// * which is called after a user clicks the done button in the
	// ContactSelector container. It gives you an hashtable of selected contacts
	// which you can loop through to get
	// * each contact. Sample code is available below.
	// *
	// * The cancelContactSelection method is called when a user press the
	// cancel button - meaning no contact was selected.
	// */
	...
	// // System.out.println(con.getBirthday()+ "," + con.getDisplayName() +
	// con.getFamilyName() + con.getFirstName() + con.getId() + con.getId() +
	// con.getNote() +
	// // con.getPrimaryEmail() + con.getPrimaryPhoneNumber() +
	// con.getAddresses() + con.getEmails() + con.getPhoneNumbers()
	// +con.getPhoto() + con.getUrls());
	//
	// }
	// }
	//
	...
	protected boolean initListModelAllContactsMultiList(final List cmp) {
		...
		// Contact contact =
		// Display.getInstance().getContactById(phoneContacts[i]);
		//
		// text2 = text2 + i;
		//
		// Hashtable h = new Hashtable();
		// h.put("Line1", i + "." + contact.getDisplayName()); //
		// contact.getFirstName() + " " + contact.getFamilyName()
	...
	protected void beforeMediaPlayerForm(Form f) {
		...
		// System.out.println("UIManager.getInstance().getThemeName(): " +
		// UIManager.getInstance().getThemeName());
		// InputStream is =
		// Display.getInstance().getResourceAsStream(getClass(), "/cowboy.mp4");
		//
		// try {
	...
	// ContactSelector selector = new ContactSelector(r, selectedTheme,
	// selectedLanguage, selectedFont); // diko mou
	...
	// is =
	// com.codename1.io.FileSystemStorage.getInstance().openInputStream(path);
	...
	public String rimRoots() {
		String result = "q";

		FileSystemStorage fs = FileSystemStorage.getInstance();
		String root = fs.getAppHomePath();

		char sep = fs.getFileSystemSeparator();
		// FileSystemStorage.getInstance().mkdir(root + sep + "test");
		// FileSystemStorage.getInstance().mkdir("file:///accounts/1000/removable/test2");
		// FileSystemStorage.getInstance().mkdir("file:///accounts/1000/shared/downloads/test3");

		String[] rootArray = fs.getRoots();
		for (int i = 0; i < rootArray.length; i++) {
			result = result + "-" + i + "-" + rootArray[i];
		}

		return "root:" + root + "result:" + result + "NEW:" + root + sep + "test";
	}
	...
	protected boolean onStorageFormSave() {
		...
		// // o1 =
		// FileSystemStorage.getInstance().openOutputStream("file:///C:/Photos/file.txt");
		// o2 =
		// FileSystemStorage.getInstance().openOutputStream("file:///C:/Photos/file.jpeg");
		// // o3 =
		// FileSystemStorage.getInstance().openOutputStream(currentImage2.getImageName());
		//
		// findArea().setText("NULL??????? :" + ImageIO.getImageIO());
		// findArea().getParent().revalidate();
		//
		// ImageIO.getImageIO().save(isGlobal, o2, ImageIO.FORMAT_JPEG, 100,
		// 100, 1); //"file:///C:/Images.1.jpeg",
		//
		// } catch (IOException ex) {
		// ex.printStackTrace();
		...
		// findArea().getParent().revalidate();
		//
		// // Dialog.show("Backup error",
		// "CSV Backup file creation has failed. "+ex.getMessage(), "OK", null);
		// // return;
		// }
		...
		//
		// InputStream a =
		// FileSystemStorage.getInstance().openInputStream(photoPath);
		// step = "3.1";
		...
		// }
		//
		// ImageIO.getImageIO().save(a, //"file:///C:/Images.1.jpeg" --> NOT
		...
		//
		// } catch (Exception ex) {
		// findArea().setText("Exception: " + ex.getMessage() + "-" +
		// ex.toString());
		// findArea().getParent().revalidate();
	...
	protected boolean onStorageFormOpen() {
		...
		final Form f = Display.getInstance().getCurrent();// (Form)findByName("Storage Form",
															// null);
	...
	//
	// // ME AYTO TON KWDIKA LEITOURGOUN TA CONTACTS OK STO "As List" ALLA
	// PARAMENOUN Loading... OTAN TO GYRNAW SE "As Grid"
	...
	// //
	// // final ContactSelector contactSelector = new ContactSelector(r, this,
	// selectedTheme, selectedLanguage, selectedFont); // new
	// ContactSelector(this); TAX
	// // f.addComponent(BorderLayout.CENTER, contactSelector);
	// //
	// //// /*
	// //// * After creating the contactselector object and add it to your form,
	// you will then add a ContactListener to your object which will listen for
	// the done and cancel
	// //// * event(done event occurs after a user has finished selecting the
	// contact(s)). The doneContactSelection method in the ContactListener is
	// the corresponding done function
	...
	// //// final Enumeration outter = h.keys();
	// ////
	// //// // Loops through the hashtable and gives you the selected Contact
	// object.
	// //// while (outter.hasMoreElements()) {
	// //// // Here is one of the selected contact, do whatever you want with
	// it.
	// //// final Contact con = (Contact) h.get(outter.nextElement());
	// //// // System.out.println(con.getBirthday()+ "," + con.getDisplayName()
	// + con.getFamilyName() + con.getFirstName() + con.getId() + con.getId() +
	...
	protected void onContacts_AllContactsMultiListAction(Component c, ActionEvent event) {
		...
		// Display.getInstance().getContactById(""+selectedIndex);
		//
		// Contact selectedContact =
		// Display.getInstance().getContactById(""+testingIndex); // Phdaei ton
		// Bradd Pitt ????
		...
		if (selectedContact != null) {
			...
			// selectedContact.getPrimaryPhoneNumber(); null (NOKIA 701 with SIM
			// card)
			...
			// String info = selectedContact.getId() + ":" + homeNo + ":" +
			// mobileNo + ":" + workNo + ":" + faxNo + ":" + otherNo;
		...
		// displayContact =
		// Display.getInstance().getContactById(phoneContacts[selectedIndex]);
		// if (selectedIndex == 0) {
	...
	protected void onFileBrowser_FolderBrowserListAction(Component c, ActionEvent event) {
		...
		if (currFile.endsWith(MyFolderBrowser.DIRECTORY_SEPARATOR) || currFile.equals(MyFolderBrowser.UP_DIRECTORY)) {
			...
			if (path2.indexOf(".jpg") != -1) {
				...
				try {
					path = path2; // browser.currentDirectory; //(String)
									// evt.getSource();
					System.out.println("path " + path);
	...
	protected void beforeLoadPhotoForm(Form f) {
		...
		// AYTO LEITOURGEI KAI KANEI TO PRESENTATION APO TIW PHOTOS POU
		// TRAVIXAME
		// final MyPhotos photos = (MyPhotos)
		// Storage.getInstance().readObject("SavedPhoto");
		//
		// if (photos != null) {
		// // Display.getInstance().callSerially(new
		// SwitchPhotos(photos.getPhotos(), findPhotoLb()));
		// new SwitchPhotos(photos.getPhotos(), findPhotoLb());
		// System.out.println("FORM NAME2: " +
		// Display.getInstance().getCurrent().getName());
		// }

		// AYTO LEITOURGEI KAI DIALEGOUME EMEIS APO TON FILE BROWSER POIA PHOTO
		// THELOUME NA ANOIXOUME (thelei tropopoihsh to size - koita commented
		// out lines)
		Display.getInstance().openImageGallery(new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				try {
					...
					String path = (String) evt.getSource();
					// we are opening the image with the file handle since the
					// image
					// is large this method can scale it down dynamically to a
					// manageable
					// size that doesn't exceed the heap
					Image i = Image.createImage(path);

					findPhotoLb().setIcon(i.scaledWidth(Display.getInstance().getDisplayWidth())); // TODO:
																									// This
																									// should
																									// be
																									// in
					...
					// Label image = new
					// Label(i.scaledWidth(Display.getInstance().getDisplayWidth()
					// / 2));
	...
	protected void onMyAudioForm_MyAudioMultiListAction(final Component c, ActionEvent event) {
		...
		//
		// String[] array =
		// Display.getInstance().getAvailableRecordingMimeTypes();
		...
		if (selectedIndex == 0) {
					...
					b.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent evt) {
							...
							try {
								...
								Display.getInstance().vibrate(vibrationTime);
								// audioMedia =
								// MediaManager.createMedia((String) node,
								// false); // true (video)
				...
				public void actionPerformed(ActionEvent evt) {
					if (evt != null) {
						...
						try {
							...
							is = com.codename1.io.FileSystemStorage.getInstance().openInputStream(audioPath); // Me
																												// ayto
																												// leitourgouse
							// is =
							// com.codename1.io.Storage.getInstance().createInputStream(photoPath);
							...
							// Display.getInstance().getMediaRecorderingMimeType());
							//
							// String[] array =
							// Display.getInstance().getAvailableRecordingMimeTypes();
							// for(int i=0; i<array.length; i++) {
							...
							// Afte loading the photo I clean the InputStream
							// and recreate it in order to copy it
							// Util.cleanup(is);
							// stepDebug = 10;
							// is =
							// com.codename1.io.FileSystemStorage.getInstance().openInputStream(audioPath);
							...
							// // Saving the photo in a directory in order to
							...
							//
							// final char sep =
							// fileSystemStorage.getFileSystemSeparator();
							// sepDebug = sep;
							// stepDebug = 2;
							...
							// homePathDebug);
							//
							// photoNameDebug = homePath + "cloud4all" + sep +
							// currentImage2.getImageName()+".jpg";
							// OutputStream o =
							// FileSystemStorage.getInstance().openOutputStream(photoNameDebug);
							// stepDebug = 4;
							// System.out.println("photoNameDebug: " +
							// photoNameDebug);
							//
							// Util.copy(is, o);
							// stepDebug = 5;
			...
			// c.getComponentForm().revalidate();
			//
			// final Button playCapturedAudio = new
			// Button("listen to recorded audio");
			//
			// debugTest = "4";
			// c.getComponentForm().setTitle(debugTest);
			// c.getComponentForm().revalidate();
	...
	protected void onMyVideoForm_MyVideoMultiListAction(Component c, ActionEvent event) {
		...
		} else if (selectedIndex == 1) { // TODO: TO BE COMMENTED OUT WHEN THE
											// VIDEO COMPONENT WILL BE WORKING
											// PROPERLY
			// Display.getInstance().vibrate(vibrationTime);
	...
	protected void postImageCapture(final Form f) {
		...
		//
		// is =
		// com.codename1.io.FileSystemStorage.getInstance().openInputStream(photoPath);
		//
		// //DEBUG
		// f.setTitle(photoPath);
		//
		...
		// if (currentImage2 instanceof EncodedImage) {
		// photos.addPhoto((EncodedImage) currentImage2);
		// System.out.println("photos.getPhotos().length: " +
			...
			public void actionPerformed(ActionEvent evt) {
				if (evt != null) {
					...
					try {
						...
						is = com.codename1.io.FileSystemStorage.getInstance().openInputStream(photoPath); // Me
																											// ayto
																											// leitourgouse
						// is =
						// com.codename1.io.Storage.getInstance().createInputStream(photoPath);
						...
						// Afte loading the photo I clean the InputStream and
						// recreate it in order to copy it
						Util.cleanup(is);
						is = com.codename1.io.FileSystemStorage.getInstance().openInputStream(photoPath);
						...
						// // Saving the photo in a directory in order to have
						// them permanently
						// stepDebug = 11;
						// Storage storage = Storage.getInstance();
						// stepDebug = 12;
	...
	protected void postVideoCapture(final Form f) {
		...
		// Xalia poiothta Video kai otan pataw gia na stamathsei na katagrafei
		// to video
	...
	// protected void onSettings_TestButtonAction(Component c, ActionEvent
	// event) {
	// // If the resource file changes the names of components this call will
	// break notifying you that you should fix the code
	...
	protected void onVideoView_ButtonAction(Component c, ActionEvent event) {
		...
		if (videoMedia != null) {
			try {
				if (videoMedia.isPlaying()) { // EDV NA ALLAZV KAI ONOMA STO
												// KOUMPI KATHE FORA
	...
	// GoogleMapsProvider("AIzaSyA0R1Bq4_ldMQQVKcBv3fq1dakPfmpUcVU"));
	//
	...
	protected void onAudioListen_AudioPlayButtonAction(Component c, ActionEvent event) {
		...
		if (audioMedia != null) {
			if (audioMedia.isPlaying()) { // EDV NA ALLAZV KAI ONOMA STO KOUMPI
											// KATHE FORA
				audioMedia.pause();
				((Button) c).setText("play");
				findAudioLabel().setIcon(r.getImage("cassetteSteady.png"));
	...
	protected void postLogin(Form f) {
		...
		// findLoginIconContainer().getSelectedStyle()
		// .setMargin(Container.TOP, 20);
		// findLoginIconContainer().getUnselectedStyle().setMargin(Container.TOP,
		// 20);
		// findLoginIconContainer().getPressedStyle().setMargin(Container.TOP,
		// 20);
		//
	...
	// protected void onShare_GeocodingButtonAction(Component c, ActionEvent
	// event) {
	// // If the resource file changes the names of components this call will
	// break notifying you that you should fix the code
	...
	// Vector responseVec =
	// gs.parseXMLResponseWithCodenameOneParser(responseStream);
	// System.out.println("responseVec: " + responseVec);
	// // ps.showResturantsOnMap(f, findProximityMc(), poiVec,
	...
	protected void postSelectAddressForm(Form f) {
		...
		InputStream responseStream = gs.requestGeocodingService(manualPostalAddress); // 12
																						// Pipinou
																						// -
																						// Thessaloniki
																						// Greece
	...
	protected void beforeMyContactsForm(Form f) {
		...
		// // Xwris ayta kanontas scroll fainontai ta contacts (prin to srcoll
		// kolane sto Loading..)
		// // Den leitourgei gia includeNativeBool = true
		// // Me ayta h symperifora einai idia alla mou fainetai kathysterei
		// ligo perissotero
	...
	protected boolean initListModelContactsTestMultiList(final List cmp) {
		...
		// item1.put("Line1", "Google");
		// item1.put("icon",
		// UIManager.getInstance().getThemeImageConstant("googleImage"));
		// item1.put("emblem", r.getImage("Arrow.png"));
		//
		// Hashtable item2 = new Hashtable();
		// item2.put("Line1", "Open Street");
		// item2.put("icon",
		// UIManager.getInstance().getThemeImageConstant("openStreetImage"));
		// item2.put("emblem", r.getImage("Arrow.png"));
		//
		// items.add(item1);
		// items.add(item2);
...
//		findTestButton().setEnabled(false);
		...
		// (MyNfcListener)NativeLookup.create(MyNfcListener.class);
		// if(myListener!=null){
		// myListener.startListener();
		// }
	}
	...
	// public static String setUI(String content) {
	// if (content != null) {
	// Log.p("beforeQRLogin5");
	// if (content.equals("1")) {
	// Log.p("beforeQRLogin6");
	// // text = text + "1";
	// // qrCode.setText(text);
	//
	// selectedLanguage = "en";
	...
	// r.getTheme("mitsosYellow"));
	// selectedTheme = "mitsosYellow";
	// Display.getInstance().getCurrent().refreshTheme();
	//
	// String currentFormName = Display.getInstance().getCurrent()
	// .getName();
	//
	// // text = text + currentFormName;
	//
	...
	// .setThemeProps(r.getTheme("mitsosWhite"));
	// selectedTheme = "mitsosWhite";
	// Display.getInstance().getCurrent().refreshTheme();
	//
	// // showForm("Main", null);
	...
	protected void onSettings_GermanRadioButtonAction(Component c, ActionEvent event) {
		// If the resource file changes the names of components this call will
		// break notifying you that you should fix the code
		super.onSettings_GermanRadioButtonAction(c, event);
